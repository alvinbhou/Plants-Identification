...
log_name = 'inception_log.csv'
save_model_name = 'inception_model.h5'

train_path = 'data/train'
valid_path = 'data/valid'

train_batches = ImageDataGenerator().flow_from_directory(train_path, target_size=(256,256), batch_size=32)
valid_batches = ImageDataGenerator().flow_from_directory(valid_path, target_size=(256,256), batch_size=32)
...
def create_InceptionResNetV2_model( w = 'imagenet', trainable = False):
    model = InceptionResNetV2(weights=w, include_top=False, input_shape=(256, 256, 3))
    if(not trainable):
        for layer in model.layers:
            layer.trainable = False
    return model
